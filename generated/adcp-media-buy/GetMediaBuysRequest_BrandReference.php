...
/**
 * Class GetMediaBuysRequest_BrandReference
...
class GetMediaBuysRequest_BrandReference implements JSONModelInterface, JsonSerializable, SerializationInterface
{
    use SerializableTrait;

    
        
            #[JsonPointer('/properties/account/oneOf/1/properties/brand/properties/domain')]
        ...
        protected $domain;
            ...
            #[JsonPointer('/properties/account/oneOf/1/properties/brand/properties/brand_id')]
        ...
        protected $brandId;
            ...
            #[JsonPointer('/properties/account/oneOf/1/properties/brand/properties/industries')]
...
**Merge semantics (normative).** The merge is **field-level**, not whole-object replacement. Each field within `brand_kit_override` (`logo`, `colors`, `voice`, `tagline`) is evaluated independently — when a field is present on the override the override value applies; when a field is absent the brand.json value applies (or is absent if brand.json doesn't carry one either). For composite fields (`colors.primary`, `colors.secondary`, `colors.accent`), the merge is one level deeper: each color slot is evaluated independently — a producer can override `colors.primary` while still inheriting `colors.secondary` from brand.json. SDKs MUST NOT treat a present `brand_kit_override.colors` as wiping the brand.json `colors` block entirely; only the per-slot fields present in the override take precedence. Without this rule, a partial-override semantics would diverge across SDKs and produce inconsistent rendering for the same payload. */
  ...
  4 => 'brand_kit_override',
    ...
    /**
     * GetMediaBuysRequest_BrandReference constructor.
     *
...
if ($additionalProperties =  (static function () use ($modelData): array {
  ...
  0 => 'GetMediaBuysRequest_BrandReference',
...
if ((!is_array($value) && !$value instanceof UnitEnum) ) {
  ...
  1 => 'array',
                 ...
                 *
                 * @return GetMediaBuysRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078|null
                 */
                public function getDataSubjectContestation(): ?GetMediaBuysRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078
                {
                 ...
                 *
                 * @param GetMediaBuysRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078 $dataSubjectContestation Inline override for the brand's contestation contact point. Useful when the operator does not control brand.json but needs to discharge Art 22(3) for this plan. brand.json is canonical; when omitted, governance agents resolve brand → house → missing.
                 *
                 * @throws ErrorRegistryException
                 *
                 * @return static
                ...
                public function setDataSubjectContestation(
                    GetMediaBuysRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078 $dataSubjectContestation
                ): static {
    ...
    try {
        return is_array($value) ? new GetMediaBuysRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078($value) : $value;
...
if (is_object($value) && !($value instanceof \Exception) && !($value instanceof GetMediaBuysRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078)) {
    $this->_errorRegistry->addError(new \PHPModelGenerator\Exception\Object\InvalidInstanceOfException($value ?? null, ...array (
  0 => 'data_subject_contestation',
  1 => 'GetMediaBuysRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078',
)));
}
...
**Merge semantics (normative).** The merge is **field-level**, not whole-object replacement. Each field within `brand_kit_override` (`logo`, `colors`, `voice`, `tagline`) is evaluated independently — when a field is present on the override the override value applies; when a field is absent the brand.json value applies (or is absent if brand.json doesn't carry one either). For composite fields (`colors.primary`, `colors.secondary`, `colors.accent`), the merge is one level deeper: each color slot is evaluated independently — a producer can override `colors.primary` while still inheriting `colors.secondary` from brand.json. SDKs MUST NOT treat a present `brand_kit_override.colors` as wiping the brand.json `colors` block entirely; only the per-slot fields present in the override take precedence. Without this rule, a partial-override semantics would diverge across SDKs and produce inconsistent rendering for the same payload.
                 *
                 * @return GetMediaBuysRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4|null
                 */
                public function getBrandKitOverride(): ?GetMediaBuysRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4
                {
                 ...
                 *
                 * @param GetMediaBuysRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4 $brandKitOverride Inline override for brand-kit fields normally resolved from `/.well-known/brand.json` on `domain` (logo, colors, voice, tagline). Use when brand.json is missing, stale, or inappropriate for this specific call — e.g., a campaign-scoped tagline, a co-branded creative, a freshly-rebranded color palette the brand.json hasn't shipped yet. Same inline-override pattern as `industries` and `data_subject_contestation` above: brand.json is canonical, the override is per-call. Adopters needing to override fields outside this subset (`voice_attributes`, `prohibited_terms`, etc.) MUST publish a different brand.json and reference it via a different `domain` — the inline override is intentionally narrow to a small high-traffic subset.
...
**Merge semantics (normative).** The merge is **field-level**, not whole-object replacement. Each field within `brand_kit_override` (`logo`, `colors`, `voice`, `tagline`) is evaluated independently — when a field is present on the override the override value applies; when a field is absent the brand.json value applies (or is absent if brand.json doesn't carry one either). For composite fields (`colors.primary`, `colors.secondary`, `colors.accent`), the merge is one level deeper: each color slot is evaluated independently — a producer can override `colors.primary` while still inheriting `colors.secondary` from brand.json. SDKs MUST NOT treat a present `brand_kit_override.colors` as wiping the brand.json `colors` block entirely; only the per-slot fields present in the override take precedence. Without this rule, a partial-override semantics would diverge across SDKs and produce inconsistent rendering for the same payload.
                ...
                public function setBrandKitOverride(
                    GetMediaBuysRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4 $brandKitOverride
                ): static {
            ...
            protected function processBrandKitOverride(array $modelData): void
            {
                ...
                $value = (function ($value) {
    try {
        return is_array($value) ? new GetMediaBuysRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4($value) : $value;
    } catch (\Exception $instantiationException) {
...
if (is_object($value) && !($value instanceof \Exception) && !($value instanceof GetMediaBuysRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4)) {
    $this->_errorRegistry->addError(new \PHPModelGenerator\Exception\Object\InvalidInstanceOfException($value ?? null, ...array (
  0 => 'brand_kit_override',
  1 => 'GetMediaBuysRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4',
)));
}
