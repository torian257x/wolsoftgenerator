...
/**
 * Class GetMediaBuyDeliveryRequest_BrandReference
...
class GetMediaBuyDeliveryRequest_BrandReference implements JSONModelInterface, JsonSerializable, SerializationInterface
{
...
**Merge semantics (normative).** The merge is **field-level**, not whole-object replacement. Each field within `brand_kit_override` (`logo`, `colors`, `voice`, `tagline`) is evaluated independently — when a field is present on the override the override value applies; when a field is absent the brand.json value applies (or is absent if brand.json doesn't carry one either). For composite fields (`colors.primary`, `colors.secondary`, `colors.accent`), the merge is one level deeper: each color slot is evaluated independently — a producer can override `colors.primary` while still inheriting `colors.secondary` from brand.json. SDKs MUST NOT treat a present `brand_kit_override.colors` as wiping the brand.json `colors` block entirely; only the per-slot fields present in the override take precedence. Without this rule, a partial-override semantics would diverge across SDKs and produce inconsistent rendering for the same payload. */
  ...
  4 => 'brand_kit_override',
    ...
    /**
     * GetMediaBuyDeliveryRequest_BrandReference constructor.
     *
...
if ($additionalProperties =  (static function () use ($modelData): array {
  ...
  0 => 'GetMediaBuyDeliveryRequest_BrandReference',
...
if ((!is_array($value) && !$value instanceof UnitEnum) ) {
  ...
  1 => 'array',
                 ...
                 *
                 * @return GetMediaBuyDeliveryRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078|null
                 */
                public function getDataSubjectContestation(): ?GetMediaBuyDeliveryRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078
                {
                ...
                public function setDataSubjectContestation(
                    GetMediaBuyDeliveryRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078 $dataSubjectContestation
                ): static {
    ...
    try {
        return is_array($value) ? new GetMediaBuyDeliveryRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078($value) : $value;
...
if (is_object($value) && !($value instanceof \Exception) && !($value instanceof GetMediaBuyDeliveryRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078)) {
    $this->_errorRegistry->addError(new \PHPModelGenerator\Exception\Object\InvalidInstanceOfException($value ?? null, ...array (
  0 => 'data_subject_contestation',
  1 => 'GetMediaBuyDeliveryRequest_Data_Subject_Contestation6ce85a378238546858b122363ebca078',
)));
}
...
**Merge semantics (normative).** The merge is **field-level**, not whole-object replacement. Each field within `brand_kit_override` (`logo`, `colors`, `voice`, `tagline`) is evaluated independently — when a field is present on the override the override value applies; when a field is absent the brand.json value applies (or is absent if brand.json doesn't carry one either). For composite fields (`colors.primary`, `colors.secondary`, `colors.accent`), the merge is one level deeper: each color slot is evaluated independently — a producer can override `colors.primary` while still inheriting `colors.secondary` from brand.json. SDKs MUST NOT treat a present `brand_kit_override.colors` as wiping the brand.json `colors` block entirely; only the per-slot fields present in the override take precedence. Without this rule, a partial-override semantics would diverge across SDKs and produce inconsistent rendering for the same payload.
                 *
                 * @return GetMediaBuyDeliveryRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4|null
                 */
                public function getBrandKitOverride(): ?GetMediaBuyDeliveryRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4
                {
...
**Merge semantics (normative).** The merge is **field-level**, not whole-object replacement. Each field within `brand_kit_override` (`logo`, `colors`, `voice`, `tagline`) is evaluated independently — when a field is present on the override the override value applies; when a field is absent the brand.json value applies (or is absent if brand.json doesn't carry one either). For composite fields (`colors.primary`, `colors.secondary`, `colors.accent`), the merge is one level deeper: each color slot is evaluated independently — a producer can override `colors.primary` while still inheriting `colors.secondary` from brand.json. SDKs MUST NOT treat a present `brand_kit_override.colors` as wiping the brand.json `colors` block entirely; only the per-slot fields present in the override take precedence. Without this rule, a partial-override semantics would diverge across SDKs and produce inconsistent rendering for the same payload.
                ...
                public function setBrandKitOverride(
                    GetMediaBuyDeliveryRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4 $brandKitOverride
                ): static {
            ...
            protected function processBrandKitOverride(array $modelData): void
            {
                ...
                $value = (function ($value) {
    try {
        return is_array($value) ? new GetMediaBuyDeliveryRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4($value) : $value;
    } catch (\Exception $instantiationException) {
...
if (is_object($value) && !($value instanceof \Exception) && !($value instanceof GetMediaBuyDeliveryRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4)) {
    $this->_errorRegistry->addError(new \PHPModelGenerator\Exception\Object\InvalidInstanceOfException($value ?? null, ...array (
  0 => 'brand_kit_override',
  1 => 'GetMediaBuyDeliveryRequest_Brand_Kit_Overridecb3f88e49db9b91749c51c9cd1b3e4d4',
)));
}
